...
public class InventoryGrid
{
    ...
    public int Сapacity => _slots.Count;
    ...
    public AddItemsToInventoryGridResult AddItems(InventoryItemConfig config, int amount)
    {
        ...
        if (config.IsStackable)
        {
            foreach (var slot in _slots)
            {
                ...
                if (slot.Config.ItemId == config.ItemId && slot.Amount < config.MaxCapacity)
                {
    ...
    public void AddItems(InventoryItemConfig config, int amount, int indexSlot)
    {
        ...
        else if (slot.Config == config && config.IsStackable && slot.Amount < config.MaxCapacity)
        {
            int canAdd = config.MaxCapacity - slot.Amount;
    ...
    public void RemoveItems(InventoryItemConfig config, int amount)
    {
        foreach (var slot in _slots)
        {
            ...
            if (slot.Config.ItemId == config.ItemId)
            {
    ...
    public void SwapItems(int fromIndex, int toIndex)
    {
        ...
        if (fromSlot.Item != null && toSlot.Item == fromSlot.Item && fromSlot.Config.IsStackable)
        {
            int canAdd = fromSlot.Config.MaxCapacity - toSlot.Amount;
        ...
        InventoryItemConfig tempItem = fromSlot.Config;
        int tempAmount = fromSlot.Amount;

        fromSlot.Set(toSlot.Config, toSlot.Amount);
